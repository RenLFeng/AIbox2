<template>
  <el-dialog
    ...
    @close="closeDialog"
  >
    <el-form ref="form" :model="formConfig" :rules="rules" size="small">
      <el-row>
        <el-col :span="19">
          ...
          <el-row>
            <el-col class="identifyRadio">
              ...
              <el-form-item label="识别方向：">
                <el-radio-group
                  v-model="formConfig.direction"
                  @change="radioDirectionChange"
                >
                  <el-radio
                    label="right"
                  >从左到右
...
export default {
  ...
  methods: {
    ...
    getMoveXY(curOffsetX, curOffsetY) {
      if (this.moveEndXY) {
        const { x, y } = this.moveEndXY;
        if (Math.abs(curOffsetX - x) >= 5 || Math.abs(curOffsetY - y) >= 5) {
          this.maxSpeed = true;
        } else {
          this.maxSpeed = false;
        }
